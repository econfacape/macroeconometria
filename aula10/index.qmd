...
ggplot(df_vol, aes(x = date, y = ht)) +
  ...
  xlab('Data') +
  ggtitle('Volatilidade estimada pelo modelo GARCH(1,1)')
...
# Adiciona uma coluna de log retorno
library(tsibbledata)
library(fable)
library(feasts)
library(fabletools)
...
Box.test(retorno, lag = 12, type = 'Ljung')
...
y_quad <- y^2
Box.test(y_quad, lag = 12, type = 'Ljung')
...
archspec <- rugarch::ugarchspec(mean.model = list(armaOrder = c(0,0), include.mean = FALSE),
                        variance.model = list(model = "sGARCH",
                                              garchOrder = c(1,0)),
                        distribution.model = "norm")

archfit <- rugarch::ugarchfit(data = retorno, spec = archspec)
...
# Estimação do modelo GARCH - 2

```{r econ9_8, warning=FALSE, message=FALSE}

garchspec <- rugarch::ugarchspec(mean.model = list(armaOrder = c(0,0), include.mean = TRUE),
                        variance.model = list(model = "sGARCH", garchOrder = c(1,1)),
                        distribution.model = "norm")

garchfit <- rugarch::ugarchfit(data = retorno, spec = garchspec)
