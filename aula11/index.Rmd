...
É assumido que os **erros estruturais**, representados por $\epsilon_t$, são um ruído branco e que a matriz de coeficientes $B^*$ corresponde aos coeficientes estruturais, que irão diferir da forma reduzida representada anteriormente (Aula 10) se $A \neq I$, onde $I$ é a matriz-identidade. Tem-se o denominado VAR estrutural ou SVAR. Se $A = I$, tem-se a exclusão dos efeitos contemporâneos entre as variáveis e se volta no VAR padrão. 

Apesar do modelo VAR padrão conseguir captar as características dinâmicas de séries temporais multivariadas, identifica-se a necessidade de transformar a matriz de variância e covariâncias dos erros para se ter choques ortogonais, isto é, não correlacionados, para a análise correta das funções de impulso-resposta e decomposição do erro de previsão.
...
Considere $e_t$ os choques/inovações originais do VAR padrão (erros correlacionados): 
...
O modelo VAR multiplicado por P é um novo modelo, com um novo erro $\mu_t$ e com o lado esquerdo da equação incorporando relações contemporâneas entre as variáveis do sistema. É conhecido como VAR Estrutural, que permite a análise de funções impulso-resposta ortogonais, decomposição da variância do erro de previsão e estimativas de coeficientes de relações contemporâneas entre as variáveis. 
...
Para se ter **identificação** é preciso impor restrições no sistema estrutural (matriz A). O número de restrições necessário para alcançar identificação depende da relação entre o número de coeficientes estimados na forma reduzida e o número de coeficientes a serem obtidos na forma estrutural; é um problema de número de incógnitas e de número de equações (relações). 

Uma solução para isto é usar a **decomposição de Cholesky** forçando que o triangulo superior seja igual a zero. Contudo, com dados reais o recomendado é que se use a Teoria Econômica para definir as restrições. 
...
**Elementos a serem identificados:** Temos $k^2$  elementos de A mais k elementos da matriz B (diagonal) de variâncias dos erros do modelo estrutural, dando um total de  $k^2+k$ (k é uma das dimensões da matriz A ou B). Contudo, impondo-se a restrição de que a matriz de relações contemporâneas tenha $1's$  na diagonal, tem-se $(k^2-k)+k=k^2$ valores desconhecidos no modelo estrutural. 
...
A idéia é modelar as relações contemporâneas entre as variáveis diretamente pela matriz $A$ considerando $B=I_k$, isto é:
...
Ao invés de modelar diretamente as relações contemporâneas entre as variáveis, especifica-se as relações entre os erros identificando-se os choques estruturais diretamente pelos choques da forma reduzida. A matriz $A$ é considerada identidade, isto é, $A=I_k$. Logo,
...
SVAR_est_a <- SVAR(var_est, Amat = amat, max.iter = 1000)
...
# Estimate
SVAR_est_b <- SVAR(var_est, Bmat = bmat)
...
Os modelos AB são estimados de maneira semelhante aos modelos A e B, especificando uma matriz Amat e uma Bmat, que contém restrições nas matrizes estruturais A e B.

```{r econ11_6, warning=FALSE, message=FALSE }
amat <- matrix(c(1,0,0,
                 NA,1,0,
                 0,0,1),
               byrow = TRUE,
...
SVAR_est_ab <- SVAR(var_est, Amat=amat, Bmat = bmat)
