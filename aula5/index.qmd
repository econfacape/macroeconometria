...
selic_ipca <- rbcb::get_series(
    c("IPCA em 12 meses" = 13522, "Taxa SELIC" = 4189), 
    start_date = "2003-01-01",
    as = "tibble")

# Correctly combining the two tibbles
selic_ipca <- bind_rows(
  selic_ipca$`IPCA em 12 meses` %>% 
    rename(value = `IPCA em 12 meses`) %>%
    mutate(variable = "IPCA em 12 meses"),
  selic_ipca$`Taxa SELIC` %>% 
    rename(value = `Taxa SELIC`) %>%
    mutate(variable = "Taxa SELIC")
)
...
# Separating the IPCA series
ipca <- selic_ipca %>%
  filter(variable == "IPCA em 12 meses") %>%
  select(date, value) %>%
  rename(IPCA = value)

# Separating the SELIC series
selic <- selic_ipca %>%
  filter(variable == "Taxa SELIC") %>%
  select(date, value) %>%
  rename(Selic = value)

ggtsdisplay(ipca$IPCA, main='IPCA')
ggtsdisplay(selic$Selic, main='Selic')
...
ipcan.df <- ur.df(ipca$IPCA, type = 'none', lags=0)
summary(ipcan.df)

ipcad.df <- ur.df(ipca$IPCA, type = 'drift', lags=0)
summary(ipcad.df)

ipcat.df <- ur.df(ipca$IPCA, type = 'trend', lags=0)
summary(ipcat.df)
```
...
ipcan.adf <- ur.df(ipca$IPCA, type = 'none', selectlags='AIC')
summary(ipcan.adf)

ipcad.adf <- ur.df(ipca$IPCA, type = 'drift', selectlags='AIC')
summary(ipcad.adf)

ipcat.adf <- ur.df(ipca$IPCA, type = 'trend', selectlags='AIC')
summary(ipcat.adf)
```
...
ipca.pp <- ur.pp(ipca$IPCA, type = 'Z-tau', model="trend", lags='short')
summary(ipca.pp)

#Estatistica de Teste KPSS para IPCA
ipca.kpss <- ur.kpss(ipca$IPCA, type = 'tau', lags='short')
summary(ipca.kpss)
```
...
print(12*(length(ipca$IPCA)/100)^(1/4))
print(12*(length(selic$Selic)/100)^(1/4))

#Estatistica de Teste DFGLS para IPCA Com Constante

ipcaC.dfgls <- ur.ers(ipca$IPCA, type = c("DF-GLS"),
                        model = c("constant"),
                        lag.max = 15)
summary(ipcaC.dfgls)
...
ipcaT.dfgls <- ur.ers(ipca$IPCA, type = c("DF-GLS"),
                     model = c("trend"),
                     lag.max = 15)
summary(ipcaT.dfgls)
...
selicC.dfgls <- ur.ers(selic$Selic, type = c("DF-GLS"),
                      model = c("constant"),
                      lag.max = 15)
summary(selicC.dfgls)

selicT.dfgls <- ur.ers(selic$Selic, type = c("DF-GLS"),
                      model = c("trend"),
                      lag.max = 15)
...
from bcb import sgs
import pandas as pd
from plotnine import ggplot, aes, geom_line, labs, theme_minimal, theme, scale_color_manual, facet_wrap, scale_x_datetime
...
# Fetch data from 2003-01-01
data = sgs.get(series, start='2003-01-01')

# Redefinir índice para obter 'data' como uma coluna
data = data.reset_index()

# Remodele os dados para o formato longo para plotnine
data_long = pd.melt(data, id_vars=['Date'], var_name='variable', value_name='value')

# Plot usando plotnine
plot = (
    p9.ggplot(data_long, p9.aes(x='Date', y='value', color='variable')) +
    p9.geom_line(size=1) +
    p9.labs(title='Inflação vs. Taxa de Juros - Selic',
            x='',
            y='% a.a.',
            caption='Dados: SGS/BCB | Elaborado por:') +
    p9.scale_color_manual(values=['#282f6b', '#b22200']) +  # Cores Customizadas
    p9.facet_wrap('~variable', nrow=2, scales='free_y') +  
    p9.scale_x_datetime(
        breaks=pd.to_datetime(['2005-01-01', '2010-01-01', '2015-01-01', '2020-01-01', '2025-01-01']),
...
# Extrai o IPCA dos dados
ipca = data[['Date', 'IPCA em 12 meses']].copy()
ipca = ipca.rename(columns={'IPCA em 12 meses': 'value'})  # renomeia a coluna
ipca = ipca.dropna()
...
# Plota IPCA 
ipca['value'].plot(ax=axes[0], title='IPCA', color='blue')
axes[0].set_ylabel('% a.a.')

# Plota FAC
plot_acf(ipca['value'], ax=axes[1], title='FAC (Função de Autocorrelação)')

# Plota FACp
plot_pacf(ipca['value'], ax=axes[2], title='FACp (Função de Autocorrelação Parcial)', method='ywm')
...
```{python}
# Extrai SELIC dos dados
selic = data[['Date', 'Taxa SELIC']].copy()
selic = selic.rename(columns={'Taxa SELIC': 'value'})  # Renomeia a coluna
selic = selic.dropna()

# Converte coluna 'Date' para datetime e configura como index
selic['Date'] = pd.to_datetime(selic['Date'])
selic.set_index('Date', inplace=True)

# Plota IPCA FAC e FACp
fig, axes = plt.subplots(3, 1, figsize=(10, 12))

# Plota IPCA
selic['value'].plot(ax=axes[0], title='SELIC', color='blue')
axes[0].set_ylabel('% a.a.')

# Plota FAC
plot_acf(selic['value'], ax=axes[1], title='FAC (Função de Autocorrelação)')

# Plota FACp
plot_pacf(selic['value'], ax=axes[2], title='FACp (Função de Autocorrelação Parcial)', method='ywm')
...
result = adfuller(ipca['value'], autolag=None, maxlag=0, regression='n')  # 'c' é o equivalente a 'none' no R

# Exibe os resultados
print("Estatística do Teste Dickey-Fuller: {:.4f}".format(result[0]))
print("p-valor: {:.4f}".format(result[1]))
for key, value in result[4].items():
    print('Valor Crítico ({}): {:.4f}'.format(key, value))

```

```{python}
# Executa o teste Dickey-Fuller com drift (constante, sem tendência)
result_drift = adfuller(ipca['value'], autolag=None, maxlag=0, regression='c')  # 'c' para constante (drift)

# Exibe os resultados
print("Estatística do Teste Dickey-Fuller (Drift): {:.4f}".format(result_drift[0]))
print("p-valor: {:.4f}".format(result_drift[1]))
for key, value in result_drift[4].items():
    print('Valor Crítico ({}): {:.4f}'.format(key, value))

```

```{python}
# Executa o teste Dickey-Fuller com tendência (constante + tendência linear)
result_trend = adfuller(ipca['value'], autolag=None, maxlag=0, regression='ct')  # 'ct' para constante + tendência

# Exibe os resultados
print("Estatística do Teste Dickey-Fuller (Tendência): {:.4f}".format(result_trend[0]))
print("p-valor: {:.4f}".format(result_trend[1]))
for key, value in result_trend[4].items():
    print('Valor Crítico ({}): {:.4f}'.format(key, value))
```

```{python}
# Executa o teste ADF com seleção automática de defasagens pelo AIC e sem constante
result_adf = adfuller(ipca['value'], autolag='AIC', regression='n')  # 'n' para sem constante (equivalente a 'none')

# Exibe os resultados
print("Estatística do Teste ADF: {:.4f}".format(result_adf[0]))
print("p-valor: {:.4f}".format(result_adf[1]))
print("Número de defasagens selecionadas: {}".format(result_adf[2]))
...
```{python}
# Executa o teste ADF com seleção automática de defasagens pelo AIC e sem constante
result_adf_drift = adfuller(ipca['value'], autolag='AIC', regression='c')  # 'n' para sem constante (equivalente a 'none')

# Exibe os resultados
print("Estatística do Teste ADF: {:.4f}".format(result_adf_drift[0]))
print("p-valor: {:.4f}".format(result_adf_drift[1]))
print("Número de defasagens selecionadas: {}".format(result_adf_drift[2]))
for key, value in result_adf_drift[4].items():
    print('Valor Crítico ({}): {:.4f}'.format(key, value))
...
```{python}
# Executa o teste ADF com seleção automática de defasagens pelo AIC e sem constante
result_adf_trend = adfuller(ipca['value'], autolag='AIC', regression='ct')  # 'n' para sem constante (equivalente a 'none')

# Exibe os resultados
print("Estatística do Teste ADF: {:.4f}".format(result_adf_trend[0]))
print("p-valor: {:.4f}".format(result_adf_trend[1]))
print("Número de defasagens selecionadas: {}".format(result_adf_trend[2]))
for key, value in result_adf_trend[4].items():
    print('Valor Crítico ({}): {:.4f}'.format(key, value))
...
# Executa o teste Phillips-Perron com constante + tendência linear e defasagens curtas
pp_test = PhillipsPerron(ipca['value'], trend='ct', lags=3)  # 'ct' para constante + tendência, lags curtos

# Exibe os resultados
print("Estatística do Teste PP (Z-tau): {:.4f}".format(pp_test.stat))
print("p-valor: {:.4f}".format(pp_test.pvalue))
print("Número de defasagens usadas: {}".format(pp_test.lags))
...
# Executa o teste KPSS com seleção automática de defasagens
stat, p_value, lags, critical_values = kpss(ipca['value'], regression='ct', nlags='auto')  # 'auto' para defasagens automáticas

# Exibe os resultados
print("Estatística do Teste KPSS: {:.4f}".format(stat))
print("p-valor: {:.4f}".format(p_value))
print("Número de defasagens usadas: {}".format(lags))
...
dfgls_test = DFGLS(ipca['value'], lags=None, trend='c')  # 'ct' para constante + tendência. Apenas c = constante.

# Exibe os resultados
print("Estatística do Teste DF-GLS: {:.4f}".format(dfgls_test.stat))
print("p-valor: {:.4f}".format(dfgls_test.pvalue))
print("Número de defasagens selecionadas: {}".format(dfgls_test.lags))

# Exibe os valores críticos
for key, value in dfgls_test.critical_values.items():
    print('Valor Crítico ({}): {:.4f}'.format(key, value))
...
# Executa o teste DF-GLS com seleção automática de defasagens
dfgls_test = DFGLS(ipca['value'], lags=None, trend='ct')  # 'ct' para constante + tendência. Apenas c = constante.
